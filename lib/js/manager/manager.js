...
function render_champion(champion) {
	...
	for (var a in abilities) {
		...
		var render_scope = {
			name: ability['name'],
			range: is_set(ability['range']) ? ability['range'] : '',
			mana_cost: is_set(ability['mana_cost']) ? ability['mana_cost'] : '',
			cooldown: is_set(ability['cooldown']) ? ability['cooldown'] : '',
			passive: '',
			active: ''
		};
		...
		var passive = ability['passive'];
		if (is_set(passive)) {
			render_scope.passive = '<table><tbody>';

			var passive_stats = passive['stats'];
			for (var stat in passive_stats) {
				render_scope.passive += '<tr><td>Passive Effect</td><td /></tr>';
				render_scope.passive += '<tr><td>' + stat + '</td><td>' + passive_stats[stat] + '</td></tr>';
			}

			render_scope.passive += '<tr><td>Info</td><td>' + passive['info'] + '</td></tr>';
			render_scope.passive += '</table></tbody>';
		}

		var active = ability['active'];
		if (is_set(active)) {
			render_scope.active = '<table><tbody>';

			var active_stats = active['stats'];
			for (var stat in active_stats) {
				render_scope.active += '<tr><td>Active Effect</td><td /></tr>';
				render_scope.active += '<tr><td>' + stat + '</td><td>' + active_stats[stat] + '</td></tr>';
			}

			render_scope.active += '<tr><td>Info</td><td>' + active['info'] + '</td></tr>';
			render_scope.active += '</table></tbody>';
		}
...
function update_level() {
	if (is_set(manager.champion)) {
		manager.champion.level = parseInt($('#champion_level').val());
	}
